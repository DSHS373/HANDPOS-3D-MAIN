...
def capture_loop(cap0, cap1, frame_queue, stop_event):
    """
    cap0, cap1에서 계속 프레임을 읽어서 frame_queue에 (frame0, frame1)로 넣는다.
    항상 '가장 최신 프레임만' 유지하도록 maxsize=1 큐를 사용.
    """
    while not stop_event.is_set():
        ...
        if not ret0 or not ret1:
            print("Error: 캡처 스레드에서 카메라 프레임을 받아올 수 없습니다.")
            stop_event.set()
            break

        # 큐가 차 있으면 이전 프레임 버리고 최신 프레임으로 교체
        if frame_queue.full():
            try:
...
def main(cam_idx0, cam_idx1, P0, P1):
    ...
    capture_thread = threading.Thread(
        target=capture_loop,
        args=(cap0, cap1, frame_queue, stop_event),
        daemon=True
    )
    ...
    try:
        while not stop_event.is_set():
            try:
                # 최신 프레임 가져오기
                frame0, frame1 = frame_queue.get(timeout=1.0)
            except Empty:
                continue
